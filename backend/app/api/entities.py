...
def list_entities(
    ...
    """, params).fetchall()
    return [
        {"id":r[0],"name":r[1],"aliases":r[2],"type":r[3],
         "frequency":r[4],"first_seen":r[5],"last_seen":r[6]}
        for r in rows
    ]


@router.get("/{entity_id}/events")
def entity_events(entity_id: str, limit: int = 50):
    conn = get_conn()
    rows = conn.execute("""
        SELECT e.id, CAST(e.date AS VARCHAR), e.title, e.category, e.location, e.tone, e.num_sources
        FROM events e
    ...
    return [{"id":r[0],"date":r[1],"title":r[2],"category":r[3],"location":r[4],"tone":r[5],"sources":r[6]} for r in rows]


@router.get("/{entity_id}/relationships")
def entity_relationships(entity_id: str):
    """Get observable relationships for an entity with full explainability."""
    conn = get_conn()
    rows = conn.execute("""
        SELECT
            r.rel_type, r.evidence_count, r.confidence, r.explanation,
            r.first_seen, r.last_seen,
            e.id, e.canonical_name, e.entity_type
        FROM relationships r
        JOIN entities e ON (
            CASE WHEN r.entity_a = ? THEN r.entity_b ELSE r.entity_a END = e.id
        )
    ...
    return [
        {
            ...
            "entity": {"id": r[6], "name": r[7], "type": r[8]},
            "transparency": {
                "is_causal": False,
